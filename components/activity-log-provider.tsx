...
import { activityEvents } from "@/lib/http";
...
export function ActivityLogProvider({
  ...
  useEffect(() => {
    const handleActivity = (event: Event) => {
      const customEvent = event as CustomEvent<ActivityEntry>;
      log(customEvent.detail);
    };

    activityEvents.addEventListener("activity", handleActivity);
    return () => {
      activityEvents.removeEventListener("activity", handleActivity);
    };
  }, [log]);
