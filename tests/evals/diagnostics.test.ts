...
describe("CEP evals: diagnostics", () => {
  ...
  it(
    ...
    async () => {
      ...
      console.log(
        `[eval][diagnostics] start cases=${cases.length} fixturesMissing=${missingFixtures.length}${missingFixtures.length > 0 ? ` ids=${missingFixtures.map((item) => item.id).join(",")}` : ""}`
      );
      async function runCase(evalCase: (typeof cases)[number]): Promise<void> {
        ...
        console.log(
          `[eval][diagnostics] done ${evalCase.id} status=${status} durationMs=${Math.round(performance.now() - start)} schemaMatched=${schemaMatched}`
        );
      }
      ...
      if (failures.length > 0) {
        const details = failures
          ...
          .join("\n");
        throw new Error(`Diagnostics eval failures:\n${details}`);
      }
    },
    TEST_TIMEOUT_MS
  );
