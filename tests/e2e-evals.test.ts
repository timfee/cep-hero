...
function expectLiveOrToken(text: string, keyword: string) {
  ...
  if (
    ...
    lower.includes("refresh my access") ||
...
describe("CEP live evals", () => {
  const TEST_TIMEOUT_MS = Number.parseInt(
    process.env.EVAL_CHAT_TIMEOUT_MS ?? "120000",
    10
  );
  ...
  it(
    ...
    async () => {
      const prompt =
        "Connector scanning fails with service unavailable errors.";
      const resp = await callChat(prompt);
  ...
  it(
    ...
    async () => {
      const prompt =
        "Connector uploads fail due to blocked ingestion endpoints.";
      const resp = await callChat(prompt);
      expectLiveOrToken(resp.text, "connector");
    },
    TEST_TIMEOUT_MS
